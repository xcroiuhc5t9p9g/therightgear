...
export const RequireAuth: React.FC<RequireAuthProps> = ({ children, onNavigate, requiredCapability }) => {
  const { isAuthenticated, isLoading, hasEffectiveCapability } = useAuth();
  ...
  if (requiredCapability && !hasEffectiveCapability(requiredCapability)) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center space-y-4">
        ...
        <button onClick={() => onNavigate('explore')} className="px-4 py-2 bg-slate-900 text-white rounded-lg font-bold">
          Return to Explore
        </button>
      </div>
    );
